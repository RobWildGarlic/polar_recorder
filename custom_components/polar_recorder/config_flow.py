...
def _schema(hass, defaults: dict[str, Any]) -> vol.Schema:
    ...
    s: dict[Any, Any] = {
        ...
        vol.Required(CONF_TWA, default=defaults.get(CONF_TWA, "")): EntitySelector(
            EntitySelectorConfig(domain=["sensor"])
        ),
        vol.Required(CONF_TWS, default=defaults.get(CONF_TWS, "")): EntitySelector(
            EntitySelectorConfig(domain=["sensor"])
        ),
        vol.Required(CONF_BSP, default=defaults.get(CONF_BSP, "")): EntitySelector(
            EntitySelectorConfig(domain=["sensor"])
        ),
        # Binning / ranges
        vol.Optional(
            CONF_TWA_MIN, default=defaults.get(CONF_TWA_MIN, DEFAULTS[CONF_TWA_MIN])
        ): NumberSelector(NumberSelectorConfig(min=0, max=360, step=1, mode="box")),
        vol.Optional(
            CONF_TWA_MAX, default=defaults.get(CONF_TWA_MAX, DEFAULTS[CONF_TWA_MAX])
        ): NumberSelector(NumberSelectorConfig(min=1, max=360, step=1, mode="box")),
        vol.Optional(
            CONF_TWA_STEP, default=defaults.get(CONF_TWA_STEP, DEFAULTS[CONF_TWA_STEP])
        ): NumberSelector(NumberSelectorConfig(min=1, max=90, step=1, mode="box")),
        vol.Optional(
            CONF_TWS_MIN, default=defaults.get(CONF_TWS_MIN, DEFAULTS[CONF_TWS_MIN])
        ): NumberSelector(NumberSelectorConfig(min=0, max=100, step=0.1, mode="box")),
        vol.Optional(
            CONF_TWS_MAX, default=defaults.get(CONF_TWS_MAX, DEFAULTS[CONF_TWS_MAX])
        ): NumberSelector(NumberSelectorConfig(min=1, max=100, step=0.1, mode="box")),
        vol.Optional(
            CONF_TWS_STEP, default=defaults.get(CONF_TWS_STEP, DEFAULTS[CONF_TWS_STEP])
        ): NumberSelector(NumberSelectorConfig(min=0.5, max=20, step=0.5, mode="box")),
        # Behaviour
        vol.Optional(
            CONF_FOLD_0_180,
            default=defaults.get(CONF_FOLD_0_180, DEFAULTS[CONF_FOLD_0_180]),
        ): bool,
        vol.Optional(
            CONF_INTERPOLATE,
            default=defaults.get(CONF_INTERPOLATE, DEFAULTS[CONF_INTERPOLATE]),
        ): bool,
        # Thresholds (OPTIONAL)
        vol.Optional(
            CONF_MIN_TWS, default=defaults.get(CONF_MIN_TWS, DEFAULTS[CONF_MIN_TWS])
        ): NumberSelector(NumberSelectorConfig(min=0, max=100, step=0.1, mode="box")),
        vol.Optional(
            CONF_MIN_BSP, default=defaults.get(CONF_MIN_BSP, DEFAULTS[CONF_MIN_BSP])
    ...
    if rec_default:
        s[vol.Optional(CONF_RECORD_GATE, default=rec_default)] = EntitySelector(
            EntitySelectorConfig(
                domain=["input_boolean", "switch", "binary_sensor"], multiple=False
            )
        )
    else:
    ...
    try:
        show_smart = hass.services.has_service(
